...
void CPlayer::LoadGame(vec2 SpawnPos, int Flag, int Kills, int Time, int Armor, int w, CItems Items, bool BHitter, bool BKiller)
{
	...
	if(w & (int)pow(2, (int)WEAPON_RIFLE))
	{
	...
	if(w & (int)pow(2, (int)WEAPON_KAMIKAZE))
	{
		m_pCharacter->m_aWeapons[WEAPON_KAMIKAZE].m_Got = true;
		m_pCharacter->m_aWeapons[WEAPON_KAMIKAZE].m_Ammo = -1;
	}
	if(w & (int)pow(2, (int)WEAPON_FREEZER))
	{
		m_pCharacter->m_aWeapons[WEAPON_FREEZER].m_Got = true;
		m_pCharacter->m_aWeapons[WEAPON_FREEZER].m_Ammo = -1;
	}
...
bool CPlayer::GetWeapon(int WID)
{
	...
	if(WID == WEAPON_RIFLE && m_GameExp.m_Weapons & (int)pow(2, (int)WEAPON_FREEZER))
		return false;
	if(WID == WEAPON_FREEZER)
	{
		m_GameExp.m_Weapons &= ~(int)pow(2, (int)WEAPON_RIFLE);
		if(m_pCharacter)
...
void CPlayer::SwitchLanguage(int ClientID, int Language)
{
	m_ClientID = ClientID;
	m_Language = Language;
}

int CPlayer::GetPlayerLanguage(int ClientID)
{
	m_ClientID = ClientID;
	if(m_Language == (int)LANG_EN)
	{	
		return (int)LANG_EN;
	}
	else if(m_Language == (int)LANG_CN)
	{
		return (int)LANG_CN;
	};
}
